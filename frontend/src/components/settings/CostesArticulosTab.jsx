...
import { Search, Save, Trash2, RefreshCw, Plus, AlertTriangle, History, Sparkles, X, Loader2 } from 'lucide-react';
...
export default function CostesArticulosTab() {
  ...
  // ── Subir con IA: pega una captura (Ctrl+V) de un pedido/tarifa de proveedor
  // + una orden en texto libre (ej. "sube todo y la campana como sin venderse",
  // "mete un 10% para calcular nuestro costo"). La IA propone los articulos;
  // el usuario revisa/edita y guarda solo lo que marque.
  const [ia, setIa] = useState(null); // { fileB64, mime, name, instruccion, loading, articulos, saving, error }
  ...
  const abrirIA = () => setIa({ fileB64: '', mime: '', name: '', instruccion: '', loading: false, articulos: null, saving: false, error: '' });

  const cargarArchivoIA = async (file) => {
    if (!file) return;
    const b64 = await leerArchivoB64(file);
    setIa(s => ({ ...s, fileB64: b64, mime: file.type, name: file.name, articulos: null, error: '' }));
  };

  // Pegar (Ctrl+V) una imagen del portapapeles cuando el modal está abierto.
  useEffect(() => {
    if (!ia) return;
    const onPaste = (e) => {
      ...
      for (let i = 0; i < clip.length; i++) {
        if (clip[i].type && clip[i].type.startsWith('image/')) {
          ...
          if (file) { e.preventDefault(); cargarArchivoIA(file); break; }
        }
      }
    };
  ...
  const analizarIA = async () => {
    if (!ia?.fileB64) { alert('Pega (Ctrl+V) o adjunta una captura primero'); return; }
    setIa(s => ({ ...s, loading: true, error: '' }));
    try {
      const r = await fetch(`${BASE}/api/rentabilidad/parse-article-costs`, {
        method: 'POST', headers: { ...authHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify({ fileBase64: ia.fileB64, instruccion: ia.instruccion }),
      });
      const data = await r.json();
      if (!data.success) throw new Error(data.error || 'No se pudo leer el documento');
      setIa(s => ({ ...s, loading: false, articulos: (data.articulos || []).map(a => ({ ...a, incluir: true })) }));
    } catch (e) {
      setIa(s => ({ ...s, loading: false, error: e.message }));
    }
  };

  const setArticuloIA = (i, campo, val) => {
    setIa(s => ({ ...s, articulos: s.articulos.map((a, x) => x === i ? { ...a, [campo]: val } : a) }));
  };

  const guardarTodosIA = async () => {
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <button onClick={abrirIA} className="px-3 py-2 bg-gradient-to-r from-fuchsia-600 to-indigo-600 text-white hover:opacity-90 rounded-lg text-xs font-bold flex items-center gap-1.5">
      ...
      {/* Modal: Subir con IA (captura + orden en texto libre) */}
      {ia && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => !ia.saving && setIa(null)}>
          <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[88vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="bg-gradient-to-r from-fuchsia-600 to-indigo-600 text-white px-5 py-3 flex items-center justify-between shrink-0">
              <h3 className="font-black text-sm uppercase flex items-center gap-2"><Sparkles size={16} /> Subir artículos con IA</h3>
              <button onClick={() => !ia.saving && setIa(null)} className="p-1.5 hover:bg-white/20 rounded-lg"><X size={18} /></button>
            </div>
            <div className="p-5 overflow-y-auto space-y-3">
              {!ia.articulos ? (
                <>
                  <label
                    className="block border-2 border-dashed border-slate-200 rounded-xl p-6 text-center text-sm text-slate-500 cursor-pointer hover:border-fuchsia-300 hover:bg-fuchsia-50/40"
                  >
                    {ia.fileB64 ? (
                      <span className="font-bold text-emerald-700">✓ {ia.name || 'Captura cargada'}</span>
                    ) : (
                      <span>Pega una captura con <b>Ctrl+V</b> (o haz clic para adjuntar un archivo)</span>
                    )}
                    <input type="file" accept="image/*,application/pdf" className="hidden"
                      onChange={e => cargarArchivoIA(e.target.files?.[0])} />
                  </label>
                  <div>
                  ...
                  <button onClick={analizarIA} disabled={ia.loading || !ia.fileB64}
                ...
                <>
                        ...
                        {ia.articulos.map((a, i) => (
                          <tr key={i} className={a.revisar ? 'bg-amber-50' : ''}>
                            <td className="p-2 text-center">
                              <input type="checkbox" checked={a.incluir} onChange={e => setArticuloIA(i, 'incluir', e.target.checked)} />
                            </td>
                            <td className="p-2">
                              <input value={a.codigo} onChange={e => setArticuloIA(i, 'codigo', e.target.value)}
                                className="w-28 px-1.5 py-1 border border-slate-200 rounded text-xs font-mono font-bold" />
                            </td>
                            <td className="p-2">
                              <input value={a.nombre} onChange={e => setArticuloIA(i, 'nombre', e.target.value)}
                            ...
                            <td className="p-2 text-right">
                              <input value={a.costeUnitario} onChange={e => setArticuloIA(i, 'costeUnitario', e.target.value)}
                                className="w-20 px-1.5 py-1 border border-slate-200 rounded text-xs text-right font-bold" />
                            </td>
                            <td className="p-2">
                              <input value={a.revisar} onChange={e => setArticuloIA(i, 'revisar', e.target.value)}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={guardarTodosIA} disabled={ia.saving}
