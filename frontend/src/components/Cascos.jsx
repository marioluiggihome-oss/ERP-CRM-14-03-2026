      ...
      <div className="flex flex-col lg:flex-row gap-5 items-start">
        ...
        <div className={`relative bg-white rounded-2xl border border-slate-200 p-4 h-fit w-full ${panelExpanded ? 'lg:flex-1' : 'lg:shrink-0'}`}
          ...
          <div className="border-t border-slate-100 pt-3 space-y-1 text-sm">
            ...
            <div className="flex justify-between text-slate-500 items-center"><span className="flex items-center gap-1">Descuento <input type="number" value={descuento} disabled={!isAdmin} title={isAdmin ? 'Editable (master)' : 'Descuento asignado por el administrador'} onChange={e => setDescuento(Number(e.target.value) || 0)} className="w-16 px-2 py-0.5 border border-slate-200 rounded text-center disabled:bg-slate-100 disabled:text-slate-400" />%</span><span className="font-bold text-rose-500">-{eur(dto)}</span></div>
