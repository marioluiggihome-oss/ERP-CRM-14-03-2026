...
/* ─── Sub-panel: Usuarios ─────────────────────────────────────────────────── */
function PanelUsuarios({ currentUser }) {
  const isMaster = !!(currentUser?.isMaster || currentUser?.isPrimaryAdmin);
      ...
      <div className="flex-1 overflow-y-auto p-5">
        <div className="flex items-center justify-between mb-3">
          <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest">
            {visibleUsers.length} usuario{visibleUsers.length !== 1 ? 's' : ''} {isMaster ? 'en el alcance seleccionado' : 'en tu organización'}
          </p>
          <button
            onClick={load}
...
function PanelEstadisticas({ currentUser }) {
  const isMaster = !!(currentUser?.isMaster || currentUser?.isPrimaryAdmin);
  ...
  const load = useCallback(async (adminId = '') => {
    ...
    try {
      ...
      if (d.success) {
        setStats(d);
        if (!adminId) setOrganizations((d.items || []).filter((item) => item.esAdmin));
      }
    } catch { /* noop */ }
    finally { setLoading(false); }
  }, []);

  useEffect(() => { load(selectedAdminId); }, [load, selectedAdminId]);
  ...
  return (
    <div className="p-4 sm:p-5 overflow-y-auto h-full">
      {isMaster && (
        <div className="mb-4">
          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Filtrar organización</label>
          <select
            value={selectedAdminId}
            onChange={(event) => setSelectedAdminId(event.target.value)}
          ...
          <table className="w-full min-w-[780px] text-sm">
            <thead className="bg-slate-50 text-slate-500">
              <tr>
                ...
                <th className="text-left p-3 text-[10px] font-black uppercase tracking-wide">Plan</th>
                <th className="text-right p-3 text-[10px] font-black uppercase tracking-wide">Cuota</th>
                <th className="text-right p-3 text-[10px] font-black uppercase tracking-wide">Usados</th>
                <th className="text-right p-3 text-[10px] font-black uppercase tracking-wide">Disponibles</th>
              </tr>
            </thead>
            <tbody>
              {stats.items.map(it => (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-hidden">
          {tab === 'usuarios' && <PanelUsuarios currentUser={currentUser} />}
          {tab === 'stats' && <PanelEstadisticas currentUser={currentUser} />}
