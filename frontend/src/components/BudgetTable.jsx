...
`.trim();
  ...
  return (
    <div className="flex flex-col h-full bg-indigo-50/10 overflow-hidden relative">
      ...
      {/* Modal para añadir tablero al presupuesto */}
      {despieceAddModal.isOpen && despieceAddModal.product && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="bg-gradient-to-r from-purple-800 to-purple-900 text-white px-6 py-4 flex items-center justify-between">
              ...
              <button 
                onClick={() => setDespieceAddModal(prev => ({...prev, isOpen: false, product: null}))}
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="bg-purple-50 rounded-xl p-4 border border-purple-200">
                <p className="text-xs font-black text-purple-800 uppercase">{despieceAddModal.product.code}</p>
                <p className="text-sm font-bold text-purple-600 mt-1">{despieceAddModal.product.name}</p>
                <div className="flex flex-wrap gap-2 mt-3">
                  <span className="px-2 py-1 bg-purple-200 text-purple-700 rounded text-[10px] font-bold">
                    {despieceAddModal.product.collection}
                  </span>
                  <span className="px-2 py-1 bg-slate-200 text-slate-700 rounded text-[10px] font-bold">
                    {despieceAddModal.product.color} - {despieceAddModal.product.finish}
                  </span>
                  <span className="px-2 py-1 bg-orange-200 text-orange-700 rounded text-[10px] font-bold">
                    {despieceAddModal.product.priceZ1?.toFixed(2)}€/m²
                  </span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <input
                      type="number"
                      value={despieceAddModal.width}
                      onChange={(e) => setDespieceAddModal(prev => ({...prev, width: Number(e.target.value)}))}
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={despieceAddModal.height}
                      onChange={(e) => setDespieceAddModal(prev => ({...prev, height: Number(e.target.value)}))}
                      className="w-full mt-1 bg-slate-50 border-2 border-slate-200 rounded-lg px-3 py-2.5 text-sm font-bold outline-none focus:border-purple-500"
                      placeholder="800"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      value={despieceAddModal.quantity}
                      min={1}
                      onChange={(e) => setDespieceAddModal(prev => ({...prev, quantity: Math.max(1, Number(e.target.value))}))}
                      className="w-full mt-1 bg-slate-50 border-2 border-slate-200 rounded-lg px-3 py-2.5 text-sm font-bold outline-none focus:border-purple-500"
                      placeholder="1"
                    />
                  </div>
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-lg font-black text-purple-700">
                      {((despieceAddModal.width / 1000) * (despieceAddModal.height / 1000) * despieceAddModal.quantity).toFixed(3)} m²
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-[9px] text-purple-500 uppercase font-bold">Precio estimado</p>
                    <p className="text-2xl font-black text-orange-600">
                      {((despieceAddModal.width / 1000) * (despieceAddModal.height / 1000) * despieceAddModal.quantity * (despieceAddModal.product.priceZ1 || 0)).toFixed(2)}€
                    </p>
                  </div>
                </div>
              </div>
