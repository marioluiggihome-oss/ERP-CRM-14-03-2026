    ...
    async def generate_render(
        ...
        if ref_b64 and is_sketch:
            ...
            task_prompt = (
                ...
                "- If NO wall units are drawn above a section, leave the upper wall completely open/clean; "
                "do NOT invent wall cabinets where none are drawn.\n"
                "- If a chimney hood / extractor is drawn on the wall above the cooktop, place a decorative "
                "stainless steel chimney hood.\n"
                "- READ HANDWRITTEN SPANISH LABELS AND SYMBOLS IN THE SKETCH:\n"
                "  · 'Micro / Horno' or 'Micro' + 'Horno': render a TALL TOWER COLUMN with a built-in OVEN and a built-in MICROWAVE directly above it.\n"
                "  · 'Frigo' / 'Frigorifico': render a real REFRIGERATOR column (modern stainless-steel freestanding fridge or panelled combi fridge with upper fridge and lower freezer doors).\n"
                "  · 'Congel.' / 'Congelador': render a tall freezer unit column.\n"
                "  · 'Columna' / '70' (Pantry/Despensero): render a tall storage cupboard column; if dividing lines (cross lines) are drawn, render the EXACT door split shown (e.g. 4 doors: 2 upper doors side-by-side and 2 lower doors side-by-side).\n"
                "  · 'Placa' / Cooktop: render a ceramic/induction glass hob on the worktop.\n"
                "  · 'Freg.' / 'Fregadero': render an undermount/inset sink basin with a modern swan-neck mixer tap.\n"
                "  · 'Ex' / 'Extraible': narrow pull-out spice/bottle rack base unit.\n"
                "  · 'Lava' / 'Lavavajillas': integrated dishwasher.\n"
                "  · 'Escob.' / 'Escobero': tall utility/broom column.\n"
                "  · Drawer & Door divisions: strictly replicate the drawn divisions on EVERY front (horizontal/vertical lines). If a column has cross division lines, render 4 doors (2 top + 2 bottom); if drawers are drawn, render individual drawer frentes.\n"
