    ...
    async def generate_render(
        ...
        if ref_b64 and is_sketch:
            ...
            task_prompt = (
                ...
                "- CONTINUOUS STRAIGHT BACK WALL (LINEAR LAYOUT): in a linear elevation, every single base cabinet, wall unit and tall column is installed against ONE SINGLE FLAT CONTINUOUS BACK WALL in a single unbroken flush line. The countertop must end directly abutting against the vertical side panel of the oven tower column. Tall columns MUST NOT be detached, freestanding, perpendicular or protruding forward into the room.\n"
                "- DOORS AND DRAWERS MUST BE FULLY CLOSED: every single cabinet door, drawer, pull-out unit and lift-up flap MUST be rendered 100% COMPLETELY CLOSED and flush in its resting position. Do NOT render open drawers, pulled-out spice racks, open flaps or partially opened doors.\n"
                "- THREE DISTINCT TALL COLUMNS: when 3 tall columns are drawn on the right, render THREE CLEARLY SEPARATED units in the same continuous wall run: 1) Oven + Microwave Tower, 2) Distinct Combi Refrigerator (stainless steel or combi split doors with top altillo), and 3) 4-Door Tall Pantry Column on the far right. Never blend the fridge into a flat blank cupboard.\n"
                "- READ HANDWRITTEN SPANISH TECHNICAL LABELS AND CODES:\n"
                "  · 'Frigo' / 'Frijo' / 'Frigorifico' / 'Combi': render a DISTINCT COMBI REFRIGERATOR appliance (modern stainless steel or panelled combi with visible upper and lower door split line + overhead altillo cabinet) placed between the oven tower and the pantry.\n"
