...
@router.get("/rentabilidad/clientes-saldo")
async def clientes_con_saldo():
    """Clientes que tienen facturas o entregas a cuenta, para el buscador del
    saldo. Devuelve codigo, nombre y cuantos documentos tiene cada uno."""
    registros = []  # (codigo, nombre, campo)
    async for f in _get_db().sale_fichas.find(
        {"docType": "factura"}, {"_id": 0, "cliente": 1, "clienteCodigo": 1}
    ):
        registros.append((str(f.get("clienteCodigo") or "").strip(),
                          str(f.get("cliente") or "").strip(), "facturas"))
    async for i in _get_db().ingresos_cuenta.find({}, {"_id": 0, "cliente": 1, "clientCode": 1}):
        registros.append((str(i.get("clientCode") or "").strip(),
                          str(i.get("cliente") or "").strip(), "ingresos"))

    # Muchas facturas antiguas se guardaron con el nombre pero SIN codigo. Si se
    # agrupara sin mas, el mismo cliente saldria dos veces (una por codigo y otra
    # por nombre). Primero se aprende que codigo corresponde a cada nombre y
    # despues se agrupa ya con el codigo puesto.
    codigo_de_nombre: Dict[str, str] = {}
    for codigo, nombre, _ in registros:
        if codigo and nombre:
            codigo_de_nombre.setdefault(nombre.upper(), codigo)

    vistos: Dict[str, Dict[str, Any]] = {}
    for codigo, nombre, campo in registros:
        if not codigo and nombre:
            codigo = codigo_de_nombre.get(nombre.upper(), "")
        if not codigo and not nombre:
            continue
        clave = codigo.upper() or nombre.upper()
        e = vistos.setdefault(clave, {"codigo": codigo, "nombre": nombre, "facturas": 0, "ingresos": 0})
        if nombre and not e["nombre"]:
            e["nombre"] = nombre
        e[campo] += 1

    items = sorted(vistos.values(), key=lambda x: (x["nombre"] or x["codigo"]).upper())
    return {"items": items, "count": len(items)}


@router.get("/rentabilidad/saldo-cliente")
async def saldo_cliente(q: str = ""):
    """Saldo de un cliente: lo facturado, lo cobrado y lo que queda pendiente,
    con el detalle de sus facturas y sus entregas a cuenta.

    Se busca por CODIGO o por NOMBRE, y las fichas se recogen por los dos
    campos: muchas facturas antiguas se guardaron con el nombre pero sin el
    codigo, y buscando solo por codigo el saldo saldria incompleto.

    Cada entrega a cuenta se cuenta UNA sola vez aunque encaje por varios
    caminos (por id de ficha, por referencia o por cliente): de lo contrario el
    saldo se inflaria con pagos duplicados.
    """
    q = (q or "").strip()
    if not q:
        raise HTTPException(status_code=400, detail="Indica el codigo o el nombre del cliente")

    # 1) Identificar al cliente. Puede no estar dado de alta en la coleccion de clientes y aun
    #    asi tener facturas: en ese caso se trabaja con lo que venga en ellas.
    esc = re.escape(q)
    cli = await _get_db().clients.find_one(
        {"codigo": {"$regex": f"^{esc}$", "$options": "i"}}, {"_id": 0, "id": 1, "codigo": 1, "nombre": 1}
    ) or await _get_db().clients.find_one(
        {"nombre": {"$regex": esc, "$options": "i"}}, {"_id": 0, "id": 1, "codigo": 1, "nombre": 1}
    )
    codigo = str((cli or {}).get("codigo") or "").strip()
    nombre = str((cli or {}).get("nombre") or "").strip()

    # 2) Fichas del cliente: por codigo exacto o por nombre.
    condiciones = []
    for valor in {v for v in (codigo, q) if v}:
        condiciones.append({"clienteCodigo": {"$regex": f"^{re.escape(valor)}$", "$options": "i"}})
    for valor in {v for v in (nombre, q) if v}:
        condiciones.append({"cliente": {"$regex": re.escape(valor), "$options": "i"}})
    fichas = await _get_db().sale_fichas.find({"$or": condiciones}, {"_id": 0}).sort("fecha", -1).to_list(2000)
    if not fichas and not cli:
        raise HTTPException(status_code=404, detail=f"No hay ningun cliente ni factura que coincida con «{q}»")
    if not nombre:
        nombre = next((f.get("cliente") for f in fichas if f.get("cliente")), "")
    if not codigo:
        codigo = next((f.get("clienteCodigo") for f in fichas if f.get("clienteCodigo")), "")

    ids_fichas = {f.get("id") for f in fichas if f.get("id")}
    refs_fichas = {_normalize_ref(f.get("ref")) for f in fichas if f.get("ref")}

    # 3) Entregas a cuenta del cliente, por cualquiera de sus tres vinculos,
    #    deduplicadas por id.
    ingresos: Dict[str, Dict[str, Any]] = {}
    async for ing in _get_db().ingresos_cuenta.find({}, {"_id": 0}):
        cc = str(ing.get("clientCode") or "").strip()
        cn = str(ing.get("cliente") or "").strip()
        suyo = (
            (ing.get("targetId") in ids_fichas)
            or (_normalize_ref(ing.get("targetRef")) in refs_fichas and ing.get("targetRef"))
            or (codigo and cc.upper() == codigo.upper())
            or (nombre and cn.upper() == nombre.upper())
        )
        if suyo:
            ingresos[ing.get("id")] = ing

    # 4) Detalle de facturas con lo cobrado en cada una.
    cobrado_map = await _cobrado_por_ficha_map()
    costes_map = await _costes_proyecto_map()
    facturas, otros_docs = [], []
    for f in fichas:
        ...
        _apply_ficha_extras(f, cobrado_map, costes_map)
        fila = {
            "id": f.get("id"), "ref": f.get("ref"), "fecha": f.get("fecha"),
            "docType": f.get("docType") or "factura",
            "venta": f["totals"]["venta"], "coste": f["totals"]["coste"],
            "margen": f["totals"]["margen"],
            "cobrado": f.get("cobrado", 0), "pendiente": f.get("pendienteCobro", 0),
            "revisada": bool(f.get("revisada")),
            "convertidoAId": f.get("convertidoAId") or "",
        }
        # Un documento ya convertido en otro (presupuesto -> factura) contaria
        # dos veces la misma venta: se lista aparte, fuera del saldo.
        if fila["docType"] == "factura" and not fila["convertidoAId"]:
            facturas.append(fila)
        else:
            otros_docs.append(fila)

    facturado = round(sum(x["venta"] for x in facturas), 2)
    ids_facturas = {x["id"] for x in facturas}
    refs_facturas = {_normalize_ref(x["ref"]) for x in facturas if x["ref"]}

    lista_ingresos, aplicado, sin_aplicar = [], 0.0, 0.0
    for ing in sorted(ingresos.values(), key=lambda i: str(i.get("fecha") or ""), reverse=True):
        importe = float(ing.get("importe", 0) or 0)
        va_a_factura = (
            ing.get("targetId") in ids_facturas
            or (bool(ing.get("targetRef")) and _normalize_ref(ing.get("targetRef")) in refs_facturas)
        )
        if va_a_factura:
            aplicado += importe
        else:
        ...
        lista_ingresos.append({
            "id": ing.get("id"), "fecha": ing.get("fecha"), "importe": round(importe, 2),
            "concepto": ing.get("concepto") or "", "metodo": ing.get("metodo") or "",
            "targetType": ing.get("targetType") or "", "targetRef": ing.get("targetRef") or "",
            "docId": ing.get("docId") or "",
            "aplicadoAFactura": bool(va_a_factura),
        })

    cobrado = round(aplicado + sin_aplicar, 2)
    return {
        "success": True,
        "cliente": {"codigo": codigo, "nombre": nombre, "id": (cli or {}).get("id", "")},
        "facturado": facturado,
        "cobrado": cobrado,
        # Lo que el cliente todavia debe. Las entregas aun no aplicadas a una
        # factura tambien descuentan: ya estan pagadas.
        "saldo": round(facturado - cobrado, 2),
        "aplicadoAFacturas": round(aplicado, 2),
        "sinAplicarAFactura": round(sin_aplicar, 2),
        "facturas": facturas,
