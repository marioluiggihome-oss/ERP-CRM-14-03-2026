...
async def clientes_con_saldo():
    ...
    async for f in db.sale_fichas.find(
    ...
    async for i in db.ingresos_cuenta.find({}, {"_id": 0, "cliente": 1, "clientCode": 1}):
...
async def saldo_cliente(q: str = ""):
    ...
    # 1) Identificar al cliente. Puede no estar dado de alta en db.clients y aun
    ...
    fichas = await db.sale_fichas.find({"$or": condiciones}, {"_id": 0}).sort("fecha", -1).to_list(2000)
    ...
    async for ing in db.ingresos_cuenta.find({}, {"_id": 0}):
